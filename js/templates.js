        ...
        items: [
            {
                ...
                name: '거래명세서',
                fields: [
                    { id: 'supply_date', label: '거래일자', type: 'date' },
                    { id: 'supplier_no', label: '공급자 등록번호', type: 'text' },
                    { id: 'supplier_name', label: '공급자 상호', type: 'text' },
                    { id: 'supplier_ceo', label: '공급자 대표성명', type: 'text' },
                    { id: 'supplier_addr', label: '공급자 주소', type: 'text' },
                    { id: 'receiver_no', label: '공급받는자 등록번호', type: 'text' },
                    { id: 'receiver_name', label: '공급받는자 상호', type: 'text' },
                    { id: 'receiver_addr', label: '공급받는자 주소', type: 'text' },
                    {
                        id: 'items',
                        label: '품목 리스트',
                        type: 'array',
                        props: [
                            { id: 'name', label: '품명', width: '2' },
                            { id: 'qty', label: '수량', type: 'number', width: '1' },
                            { id: 'price', label: '단가', type: 'number', width: '1' },
                            { id: 'remark', label: '비고', width: '1' }
                        ]
                    },
                    { id: 'remarks', label: '비고 (하단)', type: 'textarea' }
                ],
                calculate: (data) => {
                    let total = 0;
                    let taxTotal = 0;
                    ...
                    data.subtotal = total; // 공급가액
                    data.tax_total = taxTotal; // 세액
                    data.grand_total = total + taxTotal; // 합계금액
                    return data;
                },
                render: (data, mode = 'preview') => {
                    const v = (key) => mode === 'print' ? '' : (data[key] || '');
                    const money = (val) => mode === 'print' ? '' : Number(val || 0).toLocaleString();
                    const d = mode === 'print' ? '20__ . __ . __' : (data.supply_date || new Date().toLocaleDateString());

                    const items = data.items && data.items.length > 0 ? data.items : [{}, {}, {}]; // Default 3 empty rows if none

                    // Pagination Logic for Print Mode could typically go here, but for now we just render all rows.
                    // If rows > 20, it might overflow A4.
                    ...
                    const rows = items.map((item, i) => {
                        ...
                        const supply = (parseInt(qty || 0) * parseInt(price || 0)) || '';
                        const tax = supply ? Math.floor(supply * 0.1) : '';

                        if (mode === 'print' && !item.name) return '<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>';
                        ...
                        return `<tr>
                            ...
                            <td>${supply ? Number(supply).toLocaleString() : ''}</td>
                            <td>${tax ? Number(tax).toLocaleString() : ''}</td>
                            <td>${remark}</td>
                        </tr>`;
                    }).join('');

                    // Fill remaining rows to look like a full page if few items? Optional.
                    // For now, let's add a few empty rows if less than 5 for aesthetics.
                    const emptyRowsNeeded = Math.max(0, 5 - items.length);
                    const emptyRows = mode === 'print' || items.length < 5 ?
                        '<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>'.repeat(emptyRowsNeeded) : '';


                    return `
                    <div class="doc-header"><h1>거 래 명 세 서</h1><p>(공급받는자 보관용)</p></div>
                    <div class="doc-section-row" style="display:flex; gap:20px;">
                        <div class="half-box" style="flex:1; border:1px solid #000; padding:10px;">
                            <h3 style="text-align:center; border-bottom:1px solid #ccc; padding-bottom:5px; margin-bottom:5px;">공급자</h3>
                            <p>등록번호: <span class="editable">${v('supplier_no')}</span></p>
                            <p>상 호: <span class="editable">${v('supplier_name')}</span> 성명: <span class="editable">${v('supplier_ceo')}</span> (인)</p>
                            <p>주 소: <span class="editable">${v('supplier_addr')}</span></p>
                        </div>
                        <div class="half-box" style="flex:1; border:1px solid #000; padding:10px;">
                            <h3 style="text-align:center; border-bottom:1px solid #ccc; padding-bottom:5px; margin-bottom:5px;">공급받는자</h3>
                            <p>등록번호: <span class="editable">${v('receiver_no')}</span></p>
                            <p>상 호: <span class="editable">${v('receiver_name')}</span> 성명: <span class="editable" style="background:#eee;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> (인)</p>
                            <p>주 소: <span class="editable">${v('receiver_addr')}</span></p>
                        </div>
                    </div>
                    <div style="margin:20px 0; border:2px solid #000; padding:10px; text-align:center; font-weight:bold; font-size:1.2em;">
                        합계금액 (공급가액 + 세액) : 일금 [ ${money(data.grand_total)} ] 원 정 (₩ ${money(data.grand_total)})
                    </div>
                    <table class="doc-table">
                        <thead><tr><th>월/일</th><th>품목/규격</th><th>수량</th><th>단가</th><th>공급가액</th><th>세액</th><th>비고</th></tr></thead>
                        <tbody>
                            ${rows}
                            ${emptyRows}
                        </tbody>
                        <tfoot>
                            <tr style="background:#f9f9f9; font-weight:bold;">
                                <td colspan="4" style="text-align:center;">소 계</td>
                                <td>${money(data.subtotal)}</td>
                                <td>${money(data.tax_total)}</td>
                                <td></td>
                            </tr>
                        </tfoot>
                    </table>
                    <div class="doc-section">
                        <h4>[비 고]</h4>
                        <div class="doc-content-box editable" style="min-height:60px;">${(v('remarks')).replace(/\n/g, '<br>')}</div>
                    </div>
                    <div class="doc-footer" style="margin-top:20px;">
            ...
            {
                id: 'A02_purchase_order',
                name: '발주서 (주문서)',
                fields: [
                    { id: 'order_date', label: '발주일자', type: 'date' },
                    { id: 'ref_q_no', label: '참조견적번호', type: 'text' },
                    { id: 'buyer_name', label: '발주자 상호', type: 'text' },
                    { id: 'buyer_dept', label: '발주 부서/담당자', type: 'text' },
                    { id: 'buyer_tel', label: '발주자 연락처', type: 'text' },
                    { id: 'supplier_name', label: '수주자(공급자) 상호', type: 'text' },
                    { id: 'supplier_fax', label: '수주자 연락처/팩스', type: 'text' },
                    { id: 'delivery_date', label: '납기일', type: 'date' },
                    { id: 'delivery_place', label: '납품장소', type: 'text' },
                    { id: 'payment_terms', label: '결제조건', type: 'text', placeholder: '납품 후 30일 이내 현금' },
                    {
                        id: 'items',
                        label: '주문 품목 리스트',
                        type: 'array',
                        props: [
                            { id: 'name', label: '품명', width: '2' },
                            { id: 'spec', label: '규격', width: '1' },
                            { id: 'qty', label: '수량', type: 'number', width: '1' },
                            { id: 'price', label: '단가', type: 'number', width: '1' },
                            { id: 'remark', label: '비고', width: '1' }
                        ]
                    },
                    { id: 'remarks', label: '특이사항', type: 'textarea' }
                ],
                calculate: (data) => {
                    let total = 0;
                    let taxTotal = 0;
                    ...
                    data.grand_total = total + taxTotal;
                    return data;
                },
                render: (data, mode = 'preview') => {
                    const v = (key) => mode === 'print' ? '' : (data[key] || '');
                    const money = (val) => mode === 'print' ? '' : Number(val || 0).toLocaleString();
                    const d = mode === 'print' ? '20__ . __ . __' : (data.order_date || new Date().toLocaleDateString());
                    ...
                    const emptyRowsNeeded = Math.max(0, 5 - items.length);
                    const emptyRows = mode === 'print' || items.length < 5 ?
                        '<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>'.repeat(emptyRowsNeeded) : '';

                    return `
                    <div class="doc-header"><h1>발 주 서</h1><p>(Purchase Order)</p></div>
                    <div class="doc-info" style="display:flex; justify-content:space-between;">
                        <span>발주일자: ${d}</span>
                        <span>참조견적: <span class="editable">${v('ref_q_no')}</span></span>
                    </div>
                    <div class="doc-section-row" style="display:flex; gap:20px; result:20px 0;">
                        <div class="half-box" style="flex:1;">
                            <h3>■ 발주자 (Buyer)</h3>
                            <p>상 호: <span class="editable">${v('buyer_name')}</span></p>
                            <p>담 당: <span class="editable">${v('buyer_dept')}</span></p>
                            <p>연락처: <span class="editable">${v('buyer_tel')}</span></p>
                        </div>
                        <div class="half-box" style="flex:1;">
                            <h3>■ 수주자 (Supplier)</h3>
                            <p>상 호: <span class="editable">${v('supplier_name')} 귀하</span></p>
                            <p>연락처: <span class="editable">${v('supplier_fax')}</span></p>
                        </div>
                    </div>
                    <div class="doc-section" style="border:1px solid #ccc; padding:10px; margin:10px 0;">
                        <p>1. 납 기 일 : <span class="editable">${v('delivery_date')}</span></p>
                        <p>2. 납품장소 : <span class="editable">${v('delivery_place')}</span></p>
                        <p>3. 결제조건 : <span class="editable">${v('payment_terms')}</span></p>
                        <p style="font-weight:bold; margin-top:5px;">4. 합계금액 : 일금 [ ${money(data.grand_total)} ] 원 정 (₩ ${money(data.grand_total)})</p>
                    </div>
                    <table class="doc-table">
                        <thead><tr><th>No</th><th>품명/규격</th><th>수량</th><th>단가</th><th>공급가액</th><th>세액</th><th>비고</th></tr></thead>
                        <tbody>
                            ${rows}
                            ${emptyRows}
                        </tbody>
            ...
            {
                id: 'A03_delivery_note',
                name: '납품서',
                fields: [
                    { id: 'delivery_date', label: '납품일자', type: 'date' },
                    { id: 'supplier_name', label: '공급자 상호', type: 'text' },
                    { id: 'supplier_pic', label: '공급자 담당자', type: 'text' },
                    { id: 'supplier_tel', label: '공급자 연락처', type: 'text' },
                    { id: 'receiver_name', label: '인수자(납품처) 상호', type: 'text' },
                    { id: 'receiver_pic', label: '인수자 담당자', type: 'text' },
                    { id: 'receiver_tel', label: '인수자 연락처', type: 'text' },
                    { id: 'delivery_place', label: '납품장소', type: 'text' },
                    {
                        id: 'items',
                        label: '납품 품목 리스트',
                ...
                render: (data, mode = 'preview') => {
                    const v = (key) => mode === 'print' ? '' : (data[key] || '');
                    const d = mode === 'print' ? '20__ . __ . __' : (data.delivery_date || new Date().toLocaleDateString());
                    ...
                    const rows = items.map((item, i) => {
                        ...
                        if (mode === 'print' && !item.name) return '<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>';
                    ...
                    const emptyRowsNeeded = Math.max(0, 5 - items.length);
                    const emptyRows = mode === 'print' || items.length < 5 ?
                        '<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>'.repeat(emptyRowsNeeded) : '';


                    return `
                    <div class="doc-header"><h1>납 품 서</h1><p>(Delivery Note)</p></div>
                    <div class="doc-info" style="text-align:right;">
                        <span>납품일자: ${d}</span>
                    </div>
                    <div class="doc-section-row" style="display:flex; gap:20px; margin:20px 0;">
                        <div class="half-box" style="flex:1;">
                            <h3>■ 공급자 (Supplier)</h3>
                            <p>상 호: <span class="editable">${v('supplier_name')}</span></p>
                            <p>담 당: <span class="editable">${v('supplier_pic')}</span></p>
                            <p>연락처: <span class="editable">${v('supplier_tel')}</span></p>
                        </div>
                        <div class="half-box" style="flex:1;">
                            <h3>■ 인수자 (Receiver)</h3>
                            <p>상 호: <span class="editable">${v('receiver_name')}</span></p>
                            <p>담 당: <span class="editable">${v('receiver_pic')}</span></p>
                            <p>연락처: <span class="editable">${v('receiver_tel')}</span></p>
                        </div>
                    </div>
                    <div class="doc-section" style="border:1px solid #ccc; padding:10px; margin-bottom:10px;">
                        <p>납품장소: <span class="editable">${v('delivery_place')}</span></p>
                    </div>
                    <table class="doc-table">
                        <thead><tr><th>No</th><th>품명</th><th>규격</th><th>수량</th><th>비고</th><th>확인</th></tr></thead>
                        <tbody>
                            ${rows}
                            ${emptyRows}
                        </tbody>
                    ...
                    <div class="doc-footer" style="margin-top:30px;">
                        <p>위 물품을 정히 납품(인수)하였음을 확인합니다.</p>
                        <div style="display:flex; justify-content:space-between; margin-top:30px; padding:0 50px;">
                            <div class="signature">공급자: <span class="editable">${v('supplier_name')}</span> (인)</div>
                            <div class="signature">인수자: <span class="editable">${v('receiver_name')}</span> (인)</div>
                        </div>
                    </div>
                `;
                }
