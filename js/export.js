...
(function () {
   ...
   */
  async function startExport(modeOrOptions, projectDataMaybe, optionsMaybe) {
    // ✅ Compatibility:
    // - startExport(options)
    // - startExport(exportType, projectData, options)
    const normalized = normalizeExportArgs(modeOrOptions, projectDataMaybe, optionsMaybe);

    const exportType = normalizeExportType(normalized.exportType || normalized.mode || normalized.type);
    const projectData = normalized.projectData || (window.detailBuilderState && window.detailBuilderState.projectData);
    const canvas = normalized.previewCanvas || normalized.canvas || document.getElementById('previewCanvas');
    ...
    const projectName = (normalized.projectName && String(normalized.projectName).trim())
        ? String(normalized.projectName).trim()
        : prompt('프로젝트명을 입력하세요:', 'my_project');

    if (!projectName) return;

    const sliceHeight = Number(normalized.sliceHeight) || 700;

    const fmt = normalizeImageFormat(normalized.format, normalized.quality);

    try {
        // ✅ Legacy direct modes
        if (exportType === 'smartstore' || exportType === 'coupang') {
            const zip = new JSZip();
            await addCanvasSlicesToZip(zip, exportType, canvas, sliceHeight, fmt, projectName);
            await downloadZip(zip, `${projectName}_${exportType}.zip`);
            return;
        }
        ...
        // ✅ Images (Smartstore 860 + Coupang 780)
        if (exportType === 'image') {
            const zip = new JSZip();
        ...
        if (exportType === 'both') {
            ...
            await addCanvasSlicesToZip(zip, 'smartstore', canvas, sliceHeight, fmt, projectName);
            await addCanvasSlicesToZip(zip, 'coupang', canvas, sliceHeight, fmt, projectName);
            await addHTMLToZip(zip, 'html', projectName, projectData, fmt);
            await downloadZip(zip, `${projectName}_export.zip`);
            return;
        }

        alert('지원하지 않는 내보내기 타입입니다: ' + exportType);
    } catch (error) {
        console.error('Export failed:', error);
        alert('내보내기 실패: ' + (error && error.message ? error.message : '알 수 없는 오류'));
    }
}

function normalizeExportArgs(modeOrOptions, projectDataMaybe, optionsMaybe) {
    // startExport(options)
    if (modeOrOptions && typeof modeOrOptions === 'object' && !Array.isArray(modeOrOptions)) {
        const base = modeOrOptions || {};
        // Allow: startExport(options, projectData)
        if (projectDataMaybe && typeof projectDataMaybe === 'object' && !base.projectData) {
            return { ...base, projectData: projectDataMaybe };
        }
        return base;
    }

    // startExport(exportType, projectData, options)
    const exportType = (typeof modeOrOptions === 'string') ? modeOrOptions : '';
    const projectData = projectDataMaybe;
    const options = (optionsMaybe && typeof optionsMaybe === 'object') ? optionsMaybe : {};
    return { exportType, projectData, ...options };
}

function normalizeExportType(type) {
    const t = (type == null) ? '' : String(type).toLowerCase().trim();
    if (!t) return 'image';

    // legacy
    if (t === 'smartstore' || t === 'coupang') return t;

    // normalized
    if (t === 'img') return 'image';
    if (t === 'images') return 'image';
    if (t === 'png' || t === 'jpg' || t === 'jpeg' || t === 'webp') return 'image';

    if (t === 'html') return 'html';
    if (t === 'both') return 'both';
    if (t === 'image') return 'image';

    // UI sometimes uses "exportType" like "image/html/both"
    return t;
}

function normalizeImageFormat(format, quality) {
    const f = (format == null) ? 'png' : String(format).toLowerCase().trim();
    ...
    if (f === 'jpg' || f === 'jpeg' || f === 'image/jpeg') {
        ...
        mime = 'image/webp';
        ext = 'webp';
    } else if (f === 'png' || f === 'image/png') {
        mime = 'image/png';
        ext = 'png';
    }

    const q = Number(quality);
    const safeQ = Number.isFinite(q) ? Math.max(0.1, Math.min(0.98, q)) : 0.92;

    return { mime, ext, quality: safeQ };
}

/**
 * Add sliced images from a canvas into a zip folder.
 * - smartstore: width 860 (base canvas width)
 * - coupang: width 780 (scaled from 860 master)
 */
async function addCanvasSlicesToZip(zip, mode, canvas, sliceHeight, fmt, projectName) {
    if (!zip || !canvas) return;
    ...
    const baseSliceH = Math.max(200, Math.floor(sliceHeight));
    let y = 0;
    let index = 1;

    while (y < baseH) {
        const sliceHBase = Math.min(baseSliceH, baseH - y);
        const sliceHTarget = Math.max(1, Math.floor(sliceHBase * scale));
...
/**
 * Add HTML export (html/ folder) into an existing zip.
 * Uses the same generator as exportAsHTML, but does not download immediately.
 */
async function addHTMLToZip(zip, folderName, projectName, projectData, fmt) {
    if (!zip) return;

    const htmlFolder = zip.folder(folderName || 'html');

    const images = extractImages(projectData);
    const templateId = projectData.templateId || 'beauty_01';

    const htmlContent = generateHTML(projectData, templateId, images);
    const cssContent = generateCSS();

    htmlFolder.file('index.html', htmlContent);
    htmlFolder.file('styles.css', cssContent);

    if (images && images.length) {
        const imgFolder = htmlFolder.folder('images');
        for (const imgData of images) {
            const ext = (imgData && imgData.ext) ? imgData.ext : 'png';
            const blob = dataUrlToBlobSafe(imgData.dataUrl);
            if (blob) imgFolder.file(`${imgData.name}.${ext}`, blob);
        }
    }
}
