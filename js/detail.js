...
(function() {
    ...
    function measurePrice(mctx, priceData, startY, lang) {
        ...
        const boxH = Math.max(120, innerPad * 2 + textH + 6);
        ...
        return {
            ...
            height: blockHeight,
            header: { x: padX, y: headerY, text: headerText },
            box: { x: padX, y: boxY, w: boxW, h: boxH, r: boxR, innerPad, lines, lh, isPlaceholder: !textVal }
            ,
            blockBottom
        };
    }
    ...
    function drawProof(ctx, proofLayout, lang) {
        ...
        for (const line of c.r1) {
            ctx.fillText('• ' + line, c.x + c.innerPad, y);
            y += c.lh1;
        }
        y += 10;
        ...
        ctx.save();
        ctx.fillStyle = c.hasReal ? '#374151' : '#9CA3AF';
        ctx.font = `500 15px ${fontBase}`;
        ctx.textAlign = 'left';
        ctx.textBaseline = 'top';
    ...
    function drawHowto(ctx, howtoLayout, lang) {
        ...
        for (const item of howtoLayout.items) {
            ...
            // number badge
            const badgeX = item.x + 18;
            const badgeY = item.y + (item.h - 28) / 2;
            ctx.save();
            pathRoundRect(ctx, badgeX, badgeY, 28, 28, 10);
            ctx.fillStyle = '#111827';
            ctx.fill();
            ctx.restore();
            ...
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            ctx.fillText(String(item.idx), badgeX + 14, badgeY + 14);
            ctx.restore();

            // text
            ctx.save();
            ctx.fillStyle = item.isPlaceholder ? '#9CA3AF' : '#111827';
            ctx.font = `700 16px ${fontBase}`;
            ctx.textAlign = 'left';
            ctx.textBaseline = 'top';

            let y = item.y + item.padY;
            const x = item.x + 18 + 28 + 12;
    ...
    function drawFAQ(ctx, faqLayout, lang) {
        ...
        for (const line of c.qLines) {
            ctx.fillText('Q. ' + line, c.x + c.innerPad, y);
            y += c.qLH;
        }
        y += 10;
        ctx.restore();
        ...
        for (const line of c.aLines) {
            ctx.fillText('A. ' + line, c.x + c.innerPad, y);
            y += c.aLH;
        }
        ctx.restore();
    }
    ...
    function drawBrand(ctx, brandData, brandLayout, lang, seq, requestRerender) {
        ...
        ctx.save();
        ctx.fillStyle = tb.isPlaceholder ? '#9CA3AF' : '#374151';
        ctx.font = `600 16px ${fontBase}`;
        ctx.textAlign = 'left';
        ctx.textBaseline = 'top';
